...
window.onload = () => {
  ...
  const updateBurgerState = () => {
    ...
    if (width <= 1439) {
      showBurger();
      // Для мобильных устройств меняем позиционирование кнопки закрытия, так как её не видно
      if (width <= 768) {
        closeMenuNode.style.position = "fixed";
        closeMenuNode.style.top = "20px";
        closeMenuNode.style.right = "20px";
        closeMenuNode.style.zIndex = "10000";
      } else {
        closeMenuNode.style.position = "absolute";
        closeMenuNode.style.top = "-100px";
        closeMenuNode.style.right = "-100px";
      }
      return;
    }
